...
export class HttpTransportHandler {
  ...
  async connect(): Promise<void> {
    ...
    const httpServer = http.createServer(async (req, res) => {
      // Access headers for this request
      const access_token = req.headers['authorization']; // <-- All headers for this request
      ...
      try {
        // Attach access token to req.auth as per SDK signature
        (req as any).auth = { access_token };
