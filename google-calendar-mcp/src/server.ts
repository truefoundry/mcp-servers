...
export class GoogleCalendarMcpServer {
  ...
  private async executeWithHandler(handler: any, args: any, extra?: any): Promise<{ content: Array<{ type: "text"; text: string }> }> {
    if (extra && extra.access_token) {
      this.oauth2Client.setCredentials({ access_token: extra.access_token.split(" ")[1] });
    } else {
