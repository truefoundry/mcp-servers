/**
 * Helpers for handling recurring-event modification scopes (single instance,
 * all instances, this-and-following). Carried over from the upstream
 * google-calendar-mcp RecurringEventHelpers; behavior is unchanged.
 */
...
export class RecurringEventHelpers {
  ...
  updateRecurrenceWithUntil(recurrence: string[], untilDate: string): string[] {
    ...
    if (!foundRRule) throw new Error('No RRULE found in recurrence rules');
  ...
  buildUpdateRequestBody(args: any, defaultTimeZone?: string): calendar_v3.Schema$Event {
      ...
      requestBody.attendees = Array.isArray(args.attendees)
        ? args.attendees.map((attendee: any) => {
            ...
            for (const key of allowedFields) {
              if (attendee[key] !== undefined) filtered[key] = attendee[key];
            }
            return filtered;
          })
        : undefined;
    ...
    // Add timezone objects either when start/end changed, or when neither
    // changed but a new timezone was provided (timezone-only update).
