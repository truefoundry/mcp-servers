/**
 * Gmail tool registry and dispatcher. Adapted from the upstream
 * Gmail-MCP-Server. Per-request OAuth comes from the shared auth-ctx
 * layer; this module just dispatches tool calls against `ctx.getGmail()`.
 *
...
export const toolDefinitions: ToolDefinition[] = GMAIL_TOOLS.map((t) => ({
  name: t.name,
